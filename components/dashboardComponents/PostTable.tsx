...
export function PostTable({
    ...
    const [authorDetails, setAuthorDetails] = useState<Record<string, AuthorDetails>>({})

    // Fetch author details
    useEffect(() => {
        async function fetchAllAuthors() {
            const uniqueAuthorIds = Array.from(new Set(allPosts.flatMap(post => post.authors)))
            const details: Record<string, AuthorDetails> = {}
            ...
            for (const authorId of uniqueAuthorIds) {
                try {
                    ...
                    const response = await fetch(`${backend_uri}/api/users/${authorId}`, {
                        method: 'GET',
                        headers: {
    ...
    const filteredPosts = allPosts.filter(post =>
        ...
        post.authors.some(authorId => {
            const author = authorDetails[authorId] || { name: 'Unknown' }
